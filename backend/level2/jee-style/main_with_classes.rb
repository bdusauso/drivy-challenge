...
class PriceCalculator
  ...
  def calculate
    calculate_total_distance_price + calculate_total_day_price
  end

  private

  def calculate_total_distance_price
    rental.distance * car.price_per_km
  end

  def calculate_total_day_price(days = rental.duration_in_days, sum = 0)
    return sum if days.zero?

    if days > 10
      sum += car.price_per_day * 0.5
    elsif days > 4
      sum += car.price_per_day * 0.7
    elsif days > 1
      sum += car.price_per_day * 0.9
    else
      sum += car.price_per_day * 1.0
    end

    calculate_total_day_price(days - 1, sum)
  end
...
class RentalService
  ...
  def total_prices
    @rental_repo.all.map do |rental|
      ...
      total_price = PriceCalculator.new(rental, car).calculate
